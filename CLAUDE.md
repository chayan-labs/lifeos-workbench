...
Terminal-weight, agent-native **fork of Life OS**. Same Rust backend, no browser: every Life OS module renders as a TUI over the *same* declarative manifests, `lifeos-api` is linked **in-process**, and the app is simultaneously a terminal, a modal editor, an AI agent, and a Mirrorscope DAP client. Read `README.md` for the narrative spec and `docs/ARCHITECTURE.md` (+ the upstream `docs/` tree) for the authoritative **backend** spec before non-trivial work. Read `frontend/DESIGN.md` for the TUI design system.
...
- **Frontend = a second renderer backend, not new modules.** Modules stay declarative manifests (`entityTypes`/`attrs`/`views:[{kind}]`, no DOM/DB code). The Workbench interprets the *identical* manifests with **ratatui** widgets instead of React. Never fork a manifest to make it TUI-specific; if a `view.kind` can't render in a grid, degrade per `frontend/DESIGN.md`, don't change the manifest.
...
- **Three surfaces, one binary, one pane manager.** Terminal / editor / agent / Life OS are panes of a Zellij-model tiling shell. A pane is a terminal by default; switching to any other mode is instant and shares cwd + env + the in-process `lifeos-api` handle.
- **Reuse the surface, don't build it.** Editor = embed Helix core (`helix-core`/`helix-view`): rope, tree-sitter, LSP — do NOT write an editor. Agent = an **ACP** client → the existing harness — do NOT write a model. Terminal = `alacritty_terminal` VTE + `portable-pty`. Debugger = a **DAP** client → Mirrorscope — do NOT reimplement replay here.

## What changes vs upstream (the entire fork surface)
| Concern | Upstream | Here |
|---|---|---|
| UI | React+Vite SPA (Refine/Cytoscape/Tailwind) | ratatui TUI renderers over the same manifests |
...
## TUI capability boundary (be honest, don't fake it)
...
- **Visual ≈20% → escape hatches, in order:** (1) terminal graphics protocols (Kitty/Sixel/iTerm2) for real inline images — galleries, Figma thumbnails, generated media, flamegraphs (ship/recommend ghostty/kitty/wezterm); (2) on-demand external viewer for the true-graphical tail (big graphs, Figma canvas, video) so the app stays terminal-weight; (3) optional GPUI sidecar ONLY if design/graph work proves daily-critical (deliberate fallback, violates terminal-weight).
- **Out of scope for the grid:** large zoomable graphs, Figma canvas editing, inline video/audio playback, proportional-font/animation, real maps → route to hatch 2/3. Never make the whole app heavy to serve the tail.
...
1. Shell: tiling pane manager + embedded terminal + command palette. 2. Editor pane: Helix core embed. 3. Life OS TUI renderer: link `lifeos-api`, do `list`/`board`/`table` over existing manifests. 4. Agent pane (ACP) with reviewable diffs. 5. Mirrorscope DAP client (only after Mirrorscope Phases 1-2). 6. Agentic time-travel debugging. 7. Visual-tail polish (graphics protocol, external viewer). Phases 1-4 alone = shippable lightweight agentic IDE with Life OS built in.
