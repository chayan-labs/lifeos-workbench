...
Build in Rust: `lifeos-api` (single DB-token owner), `lifeos-vcs` (universal versioning), `lifeos-ingest` (media→text), `lifeos-pipelines` + Life OS Actions, `lifeos-drain` (job claim), `broker-guard`, `bin/lifeos` CLI, marketplace signing. Stays JS: SPA `core/`, module manifests, Worker bot, `scaffold.js`. Stays Python: memvec. See `docs/RUST-COMPONENTS.md`.
