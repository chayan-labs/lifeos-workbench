...
- **One UI tree, two backends.** All panes/views/keymaps are written once against ratatui; the GPU window and `--tui` are interchangeable `Backend` impls. Never write window-only UI logic above the backend seam (window-only *renderer* capabilities — inline images, sub-cell decorations, ghost text — live below it and degrade gracefully in `--tui`).
