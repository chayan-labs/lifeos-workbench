...
describe("worker fetch handler", () => {
  it("returns 200 ok on the liveness route", async () => {
    const res = await worker.fetch(new Request("https://example.com/"), { BOT_TOKEN: "fake" });
    expect(res.status).toBe(200);
    expect(await res.text()).toBe("ok");
  });

  it("returns 404 on unknown routes", async () => {
    const res = await worker.fetch(new Request("https://example.com/nope"), { BOT_TOKEN: "fake" });
    expect(res.status).toBe(404);
  });

  it("returns 404 for a GET on the telegram webhook path", async () => {
    const res = await worker.fetch(new Request("https://example.com/telegram"), { BOT_TOKEN: "fake" });
    expect(res.status).toBe(404);
  });
});
