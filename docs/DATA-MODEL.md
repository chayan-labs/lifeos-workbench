...
1. **Single-writer-per-row discipline** - the bot writes its lane (light/medium), the Mac writes its lane (heavy). Make this an explicit invariant; the tiering already pushes this way.
2. **`events` (append-only, conflict-free) is the reconciliation source of truth** - on a detected row conflict, repair entity state from the event log rather than trusting the blind last-push outcome. (`lifeos-drain`/the API implement this.)
