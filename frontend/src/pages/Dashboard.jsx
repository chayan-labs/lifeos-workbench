...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Welcome Hero */}
      <div className="neo-surface neo-border-thick neo-shadow p-6 relative overflow-hidden bg-[var(--neo-yellow)]">
        <div className="max-w-3xl flex flex-col items-start gap-4">
          <div>
            <h2 className="neo-title-lg text-black mb-2">Life OS is Live</h2>
            <p className="neo-body-lg text-black font-semibold">
          ...
          <Link to="/docs" className="neo-btn py-2.5 px-4 bg-white text-xs font-mono font-bold flex items-center gap-1.5 shadow-[2px_2px_0_0_#000]">
            VIEW SYSTEM SPECIFICATIONS PORTAL →
          </Link>
        </div>
        <div className="absolute right-4 bottom-4 opacity-10 pointer-events-none hidden lg:block">
          <Terminal size={120} className="text-black" />
        </div>
      </div>
        ...
        {stats.map((stat, i) => (
          <div key={i} className="neo-surface neo-border neo-shadow neo-shadow-hover p-5 flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div>
                <span className="neo-label-sm text-[var(--neo-text-muted)] block mb-1">{stat.name}</span>
            ...
            <div className="mt-4 pt-3 border-t-2 border-[var(--neo-border)] border-dashed flex justify-between items-center">
              <span className="neo-label-sm text-[var(--neo-text-muted)]">{stat.change}</span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 neo-surface neo-border-thick neo-shadow p-6 flex flex-col gap-6">
          <div className="flex justify-between items-center border-b-4 border-[var(--neo-border)] pb-4">
            <h3 className="neo-title-md">Architectural Topology</h3>
            <div className="flex gap-2">
              <button 
                onClick={() => setActiveTab('architecture')}
                className={`neo-btn py-1 px-3 neo-label-sm ${activeTab === 'architecture' ? 'bg-[var(--neo-yellow)]' : ''}`}
              >
                Workflow
              </button>
              <button 
                onClick={() => setActiveTab('data')}
                className={`neo-btn py-1 px-3 neo-label-sm ${activeTab === 'data' ? 'bg-[var(--neo-yellow)]' : ''}`}
              >
                Data Sync
              </button>
            </div>
          ...
          {activeTab === 'architecture' ? (
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center items-stretch mt-4">
                ...
                <div className="neo-surface neo-border p-4 flex flex-col gap-3 bg-[var(--neo-surface-muted)] relative">
                  <span className="neo-chip neo-chip--review absolute -top-3 left-4">TIER 1 (ALWAYS ON)</span>
                  <div className="flex justify-center mt-3 text-[var(--neo-blue)]"><Smartphone size={32} /></div>
                  ...
                  <div className="mt-auto pt-2 border-t border-[var(--neo-border)] text-xs font-bold text-left text-[var(--neo-red)]">
                    * Outward actions are Gated
                  </div>
                </div>

                {/* Database Bridge */}
                <div className="neo-surface neo-border p-4 flex flex-col gap-3 bg-white relative justify-center">
                  <span className="neo-chip neo-chip--completed absolute -top-3 left-4">CANONICAL LAYER</span>
                  <div className="flex justify-center text-[var(--neo-mint)]"><Database size={32} /></div>
                ...
                {/* Local Mac harness */}
                <div className="neo-surface neo-border p-4 flex flex-col gap-3 bg-[var(--neo-surface-high)] relative">
                  <span className="neo-chip neo-chip--active absolute -top-3 left-4">TIER 2 (LOCAL HEAVY)</span>
                  <div className="flex justify-center text-[var(--neo-text)]"><Laptop size={32} /></div>
              ...
              <div className="neo-surface neo-border p-4 bg-white flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="neo-tag bg-[var(--neo-yellow)]">Hot-reloaded live</div>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-6">
              ...
              <div className="neo-border p-4 bg-white neo-radius flex flex-col gap-4">
        ...
        <div className="xl:col-span-4 flex flex-col gap-6">
          <div className="neo-surface neo-border-thick neo-shadow p-5 bg-zinc-950 text-white min-h-[380px] flex flex-col justify-between">
            <div className="flex justify-between items-center border-b border-zinc-800 pb-2 mb-3">
              ...
              <div className="w-2 h-2 rounded-full bg-[var(--neo-mint)] animate-pulse" />
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3 justify-center text-xs">
              ...
              <div className="flex gap-2">
                <button className="flex-1 py-1.5 border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 font-bold text-center text-[10px]">
                  APPROVE DRAFTS
                </button>
                <button className="flex-1 py-1.5 border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 font-bold text-center text-[10px]">
                  VIEW METRICS
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 neo-surface neo-border-thick neo-shadow p-5 bg-white">
          <div className="flex justify-between items-center border-b-2 border-black pb-3 mb-4">
            ...
            <button 
              ...
              className="neo-btn py-1 px-3 bg-[var(--neo-yellow)] text-xs font-bold"
            >
          ...
          <div className="flex flex-col gap-3">
            {pipelineLogs.map((log, idx) => {
              const Icon = log.icon;
              return (
                <div key={idx} className="p-3 bg-[var(--neo-bg)] neo-border flex justify-between items-center text-xs">
                  <div className="flex items-center gap-2 font-semibold">
                    <Icon size={14} className="text-[var(--neo-blue)]" />
                    <span>{log.stage}</span>
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 border font-bold uppercase ${
                    log.status === 'done' ? 'bg-[var(--neo-mint)]' :
                    log.status === 'running' ? 'bg-[var(--neo-yellow)] animate-pulse' :
                    log.status === 'gated' ? 'bg-[var(--neo-red)] text-white' : 'bg-white text-gray-400'
                  }`}>
                    {log.status}
                  </span>
                </div>
        ...
        <div className="lg:col-span-5 neo-surface neo-border-thick neo-shadow p-5 bg-white">
          ...
          <div className="flex flex-col gap-3">
            {actions.map((act) => (
              <div key={act.id} className="p-3 bg-[var(--neo-surface-muted)] neo-border flex justify-between items-center text-xs">
                <div>
                  <span className="neo-label-sm text-[10px] text-[var(--neo-blue)] block mb-0.5">ON EVENT: {act.trigger}</span>
                ...
                <button onClick={() => toggleAction(act.id)} className="neo-icon-btn p-1 border-0 bg-transparent cursor-pointer">
                  {act.active ? (
                    <ToggleRight size={28} className="text-[var(--neo-mint)]" />
                  ) : (
